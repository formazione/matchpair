...
class Win:
	def __init__(self):
		...
		cols, rows = (8,8)
		self.tile_size = 64
		size = cols*self.tile_size, rows*self.tile_size 
	...
	def draw_screen(self):
		...
		for nrow, row in enumerate(grid.lyt_surfaces):
			for ncol, col in enumerate(row):
				self.screen.blit(col, (ncol*self.tile_size, nrow*self.tile_size))
				...
				if event.type == pygame.MOUSEBUTTONDOWN:
					...
					if grid.lyt[row][col] != [0,0,0]:
						...
						if count_taken == 1 :
							...
							pygame.draw.rect(grid.lyt_surfaces[r1][c1], (255,255,255), (1,1,62,62), 3)
