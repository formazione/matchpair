...
tile_size = 100
time_level = 2000
surf_star = pygame.image.load("resources\\star.png")
...
time_to_vanish = 0
pointer_size = 5
multiply_score = 0 # when you make more win guess in a row it gives you more points
fast_premium = 100
class Win:
    def __init__(self):
        couples = 32
    ...
    def draw_screen(self):
        ...
        self.draw_lines()
        self.screen.blit(message(f"SCORE {int(score)} - TIME {pygame.time.get_ticks()//720}"), (0,0))
        self.particles_run_glitter(random.randint(0, 8*tile_size), (255, 255, 255))
    ...
    def pick(self):
        ...
        if pygame.time.get_ticks() - start < time_level and not count_taken and self.matches < 16:
            for nrow, row in enumerate(self.grid.lyt_surfaces):
                for ncol, col in enumerate(row):
                    self.screen.blit(col, (ncol*tile_size, nrow*tile_size))
            self.particles_run_glitter(random.randint(0, 8*tile_size), (255, 255, 255))
    ...
    def game_loop(self):
        global first_try, start, attempt, score, multiply_score, fast_premium
        global music, hiding, count_taken, tiles, time_to_vanish, pointer_size
        ...
        while loop:
                            ...
                            if count_taken == 2:
                                ...
                                if (r2, c2) != (r1, c1):
                                    ...
                                    if first == second:
                                        ...
                                        score += 10*multiply_score + fast_premium
                                        multiply_score += 1
                                        play(snd_win)
                                        count_taken = 0
            ...
            # The pointer is red if the tile is empty
            if self.grid.lyt[mrow][mcol] == [0,0,0]:
                pygame.draw.circle(self.screen, (255,0,0), pygame.mouse.get_pos(), 10, 2)
            # when you look for the other pair it becomes of the color of the tile you need
            elif count_taken == 1: 
                pygame.draw.circle(self.screen, self.pointer_clr, pygame.mouse.get_pos(), 10, 10) 
                pygame.draw.circle(self.screen, (255, 255, 255), pygame.mouse.get_pos(), pointer_size*2, 10)
                self.screen.blit(message("find"), (mx-25, my-35))
                self.screen.blit(
                    pygame.transform.scale(surf1, (20,20)),
                    (mx-25, my-50))
            else:
                pygame.draw.circle(self.screen, (200, 100, 255), pygame.mouse.get_pos(), pointer_size)
                self.screen.blit(message("no"), (mx-25, my-35))
            if self.matches > 0:
                self.particles_explosion(self.pcolor)
            # make the last 2 selected vanish after a while
            pointer_size += .1
            if pointer_size > 10:
                pointer_size = 5
            if score > 0:
